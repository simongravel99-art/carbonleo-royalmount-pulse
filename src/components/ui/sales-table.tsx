...
export function SalesTable({ 
  ...
  const exportCSV = () => {
    const headers = [
      "Tenant", "Category", "GLA (sqft)", "Total Sales", "Sales/sqft", "YoY Index (%)"
    ]
          ...
          <Table>
            ...
            <TableBody>
              {filteredData.length === 0 ? (
                <TableRow>
                  <TableCell 
                    colSpan={6 + (showGROC ? 1 : 0) + (showFlags ? 1 : 0)} 
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className={cn("text-right", density === "compact" && "py-2")}>
                        <Badge 
                          variant={tenant.yoyIndex >= 100 ? "default" : "destructive"}
                          className={cn(
                            tenant.yoyIndex >= 100 
                      ...
                      {showGROC && (
                        <TableCell className={cn("text-right", density === "compact" && "py-2")}>
                          {groc !== null ? (
                            <Badge className={getGROCBadgeClass(groc, {
                              grocThresholds: { good: 10, warning: 15, danger: 15 },
                              bottomPerformersLimit: 15,
                              topPerformersLimit: 15,
                              outlierThresholds: { minSales: 5000, minGla: 300 }
                            })}>
                              {formatPct(groc / 100)}
                            </Badge>
                          ) : (
